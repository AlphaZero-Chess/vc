// ==UserScript==
// @name         Lichess Bot - AUTHENTIC ALPHAZERO v11.0.0 AGGRESSIVE CHAMPION
// @description  Authentic AlphaZero - Aggressive, Solid, and STRONG
// @author       AlphaZero Authentic Edition
// @version      11.0.0-ALPHAZERO-AGGRESSIVE
// @match         *://lichess.org/*
...
const CONFIG = {
    ...
    // SUPERHUMAN search depth - OPTIMIZED for aggressive play
    baseDepth: 40,                  // Base depth - SOLID CALCULATION
    strategicDepth: 55,             // Strategic positions - SUPERHUMAN PLANNING
    endgameDepth: 55,               // Endgame - PERFECT TECHNIQUE
    openingDepth: 45,               // Opening (40→45) - AGGRESSIVE PREPARATION
    classicalDepth: 60,             // Classical - MAXIMUM DEPTH
    winningDepth: 52,               // Winning - PERFECT CONVERSION
    tacticalDepth: 58,              // Tactical - TACTICAL MASTERY
    criticalDepth: 60,              // Critical - EMERGENCY DEPTH
    ...
    // SUPERHUMAN positional understanding (TRUE ALPHAZERO) - AGGRESSIVE
    positionWeight: 3.0,            // MAXIMUM positional play
    initiativeBonus: 105,           // ULTRA-HIGH initiative value (95→105) - FIGHT FOR ADVANTAGE
    pieceActivityBonus: 100,        // Piece activity PARAMOUNT (90→100) - AGGRESSIVE PIECES
    controlBonus: 75,               // Space and control critical
    mobilityWeight: 2.8,            // Piece mobility ESSENTIAL (2.6→2.8) - ACTIVE PLAY
    coordinationWeight: 2.5,        // Piece coordination VITAL
    
    // AUTHENTIC ALPHAZERO - Maximum creativity and freedom (AGGRESSIVE)
    sacrificeThreshold: 0.75,       // VERY willing to sacrifice for initiative
    unconventionalRate: 0.50,       // 50% unconventional - TRUE CREATIVITY
    complexPositionBonus: 0.40,     // Maximum creativity in complex positions
    longTermFocus: 0.99,            // 99% focus on long-term positional play
    eleganceThreshold: 0.55,        // Strong favor for elegant moves
    openingScoreDiffThreshold: 80,  // More flexible opening creativity
    pieceSafetyWeight: 2.8,         // Piece safety weight (3.5→2.8) - BALANCED, NOT PASSIVE
    
    // Winning conversion - AGGRESSIVE
    winningThreshold: 150,          // Conservative winning threshold
    winningCreativity: 0.70,        // HIGH creativity when winning
    accelerationBonus: 0.60,        // Strong bonus for increasing advantage
    positionalSacrifice: 0.70,      // VERY willing to sacrifice when winning
    
    // AUTHENTIC ALPHAZERO PERSONALITY - AGGRESSIVE CHAMPION
    contempt: 90,                   // HIGH contempt (60→90) - FIGHT FOR WINS
    riskTolerance: 0.95,            // MAXIMUM risk tolerance (0.90→0.95) - AGGRESSIVE PLAY
    aggressivePlanning: 0.98,       // ULTRA-AGGRESSIVE long-term planning (0.95→0.98)
    
    // Tactical detection - REMOVED FORCING (allow creativity in tactics)
    tacticalThreshold: 0.85,        // Higher threshold - less forcing
    threatResponseDepth: 2,         // Reduced - don't overreact to threats
    forcingMoveBonus: 30,           // REDUCED - allow alternatives
    evaluationDropThreshold: 200,   // INCREASED - don't panic on eval drops
    criticalEvalThreshold: -500,    // MUCH LOWER - rarely go into crisis mode
    ...
    // AUTHENTIC ALPHAZERO: Advanced positional parameters - AGGRESSIVE
    pawnStructureWeight: 1.8,       // Higher pawn structure importance
    kingSafetyWeight: 1.3,          // REDUCED (1.4→1.3) - less paranoid, more aggressive
    outpostBonus: 60,               // HIGHER bonus for piece outposts (55→60)
    spaceControlWeight: 1.8,        // HIGHER space control importance (1.7→1.8)
    prophylacticBonus: 45,          // HIGHER bonus for prophylactic moves
    nonObviousBonus: 55,            // MUCH HIGHER - favor elegant moves (50→55)
    openingCreativity: 0.45,        // AGGRESSIVE opening creativity (0.25→0.45)
...
/**
 * NEW v11.0.0: BALANCED - Detect if a move leaves pieces hanging
 * Prevents major blunders while allowing tactical sacrifices
...
function detectHangingPieces(proposedMove, alternatives) {
    try {
        ...
        // v11.0.0: INCREASED threshold (200→250) to allow tactical sacrifices
        // Only flag SEVERE blunders, not positional sacrifices
        if (scoreDrop > 250) {
            debugLog("[SAFETY]", `🚨 WARNING: Move ${proposedMove} drops eval by ${scoreDrop}cp!`);
            
            // Check if this looks like a blunder (sudden large loss)
            const isBlunder = scoreDrop > 350;  // Increased from 300
            const isSevereLoss = scoreDrop > 500;  // Increased from 400
            ...
            if (isSevereLoss) {
                ...
                debugLog("[SAFETY]", `⚠️ WARNING: Possible hanging piece (${scoreDrop}cp)`);
                return { safe: false, hangingPiece: "piece", evaluation: scoreDrop };
            }
        }
        
        // v11.0.0: REMOVED 100cp check - allow aggressive sacrifices!
        // AlphaZero sacrifices material for initiative
...
/**
 * NEW v11.0.0: Validate move safety - BALANCED approach
 * Prevents severe blunders while allowing tactical play
 */
function validateMoveSafety(move, alternatives, currentEval) {
    try {
        ...
        if (!safetyCheck.safe) {
            ...
            // v11.0.0: LESS RESTRICTIVE - only reject SEVERE blunders
            // In good positions, reject unsafe moves (but higher threshold)
            if (currentEval > -200 && safetyCheck.evaluation > 400) {
                return false;
            }
            
            // Even in bad positions, don't make it catastrophically worse (>500cp)
            if (safetyCheck.evaluation > 500) {
                return false;
            }
        }
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // AUTHENTIC ALPHAZERO: AGGRESSIVE opening play (first 10 moves) - v11.0.0
    if (moveCount <= 10) {
        debugLog("[ENGINE]", `📖 OPENING PHASE (move ${moveCount}) - AGGRESSIVE ALPHAZERO!`);
        
        // NEW v11.0.0: In opening, strongly prefer aggressive development
        // But also validate for safety (no hanging pieces in opening)
        const bestMoveSafe = validateMoveSafety(bestMove, alternatives, currentEval);
        
        if (!bestMoveSafe) {
            // Best move is unsafe - find safe aggressive alternative
            for (let i = 1; i < Math.min(alternatives.length, 3); i++) {
                if (validateMoveSafety(alternatives[i].move, alternatives, currentEval)) {
                    debugLog("[ENGINE]", `📖 OPENING: Safe aggressive alternative (best unsafe)`);
                    return alternatives[i].move;
                }
            }
        }
        
        // In opening, allow more creativity for aggressive theory
        if (scoreDiff > CONFIG.openingScoreDiffThreshold) {
            debugLog("[ENGINE]", `📖 OPENING: Clear best move (Δ${scoreDiff}), playing it`);
            return bestMove;
        }
        
        // Apply HIGH opening creativity (45% - AGGRESSIVE) v11.0.0
        if (Math.random() > CONFIG.openingCreativity) {
            debugLog("[ENGINE]", "📖 OPENING: Aggressive best move");
            return bestMove;
        }
        
        // NEW v11.0.0: Allow MORE creative alternatives for aggressive play
        if (scoreDiff < 45 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            // Prefer moves that increase piece activity (aggressive development)
            const activity = evaluatePieceActivity(currentFen);
            if (activity > 0.5 || scoreDiff < 30) {
                debugLog("[ENGINE]", `📖 OPENING: Aggressive creative alternative (Δ${scoreDiff})`);
    ...
    // AUTHENTIC ALPHAZERO: More lenient score difference threshold
    if (scoreDiff > 60 && !isWinning) {
        debugLog("[ENGINE]", "📊 Clear best move (diff: " + scoreDiff + ") - playing it");
        return bestMove;
    }
    
    // Calculate effective unconventional rate (MAXIMUM CREATIVITY)
    const effectiveUnconventionalRate = positionComplexity > 0.6 
        ? CONFIG.unconventionalRate + CONFIG.complexPositionBonus 
        : CONFIG.unconventionalRate * 0.85; // Even in simple positions, maintain creativity
    ...
    // AUTHENTIC ALPHAZERO: Maximum creativity in complex positions
    if (positionComplexity > 0.50 && scoreDiff < 55 && Math.random() < effectiveUnconventionalRate) {
...
function initializeChessEngine() {
    ...
    chessEngine.postMessage("setoption name MultiPV value 20");         // AUTHENTIC: See many options for creativity
    chessEngine.postMessage("setoption name Hash value 512");          // Increased from 256
    chessEngine.postMessage("setoption name Contempt value 90");       // v11.0.0: HIGH contempt for AGGRESSIVE fighting chess
    ...
    console.log("🤖 AUTHENTIC ALPHAZERO v11.0.0 - AGGRESSIVE CHAMPION EDITION");
    console.log("⚔️ NEW v11.0.0: AGGRESSIVE opening theory - proper Sicilian responses (Nf3, d4!)");
    console.log("⚔️ NEW v11.0.0: HIGH contempt (90) - fights for wins from move 1");
    console.log("⚔️ NEW v11.0.0: Strong initiative focus - piece activity paramount");
    console.log("⚔️ NEW v11.0.0: Maximum risk tolerance (95%) - true fighting spirit");
    console.log("⚔️ NEW v11.0.0: Passive move detection - eliminates d3, Nh3, weak moves");
    console.log("✅ AUTHENTIC: Positional sacrifices for long-term initiative");
    console.log("✅ AUTHENTIC: Elegant, non-obvious moves - self-taught creativity");
    console.log("✅ AUTHENTIC: Maintains creative style even when behind");
    console.log("✅ AUTHENTIC: No defensive panic - fights with positional understanding");
    console.log("✅ AUTHENTIC: High unconventional rate (50%) for true AlphaZero play");
    console.log("✅ AUTHENTIC: Deep line exploration - considers 3rd, 4th, 5th alternatives");
    console.log("✅ AUTHENTIC: MultiPV 20, Depth 40-60, Maximum freedom");
    console.log("✅ BALANCED: Safety checks - solid but NOT passive");
    console.log("⚔️ AUTHENTIC ALPHAZERO: AGGRESSIVE, SOLID, and STRONG!");
}
