// ==UserScript==
// @name         Lichess Bot - AlphaZero Authentic v15.0.0
// @description  True AlphaZero Style - Positional Mastery, Strategic Sacrifices, Beats Strong Engines
// @author       AlphaZero Authentic Edition
// @version      15.0.0-ALPHAZERO-AUTHENTIC
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ALPHAZERO AUTHENTIC BOT v15.0.0 - TRUE POSITIONAL MASTERY
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * NEW v15.0.0 - AUTHENTIC ALPHAZERO APPROACH:
 * 🎯 POSITIONAL MASTERY: Long-term planning with strategic sacrifices
 * 🎯 MEASURED CREATIVITY: 22% unconventional moves - elegant alternatives
 * 🎯 AGGRESSIVE THEORY: 18% opening creativity - principled aggression
 * 🎯 STRATEGIC SACRIFICES: 42% sacrifice tolerance - positional compensation
 * 🎯 CALCULATED RISKS: 58% risk tolerance - carefully aggressive
 * 🎯 EFFICIENT DEPTH: Optimized 22-30 depth - tactical precision maintained
 * 
 * CORE PLAYING PRINCIPLES v15.0.0:
 * ✅ POSITIONAL UNDERSTANDING: Trust long-term compensation (68% focus)
 * ✅ MEASURED CREATIVITY: Allow elegant, non-obvious moves
 * ✅ STRATEGIC SACRIFICES: Accept positional sacrifices for initiative
 * ✅ PIECE ACTIVITY: Prioritize mobility and coordination
 * ✅ TACTICAL SAFETY: Maintain 130cp blunder threshold
 * ✅ AGGRESSIVE OPENINGS: Principled attacking play
 * ✅ LONG-TERM PLANNING: Superior strategic depth
 * ✅ INITIATIVE FOCUS: Activity over material when compensated
 * 
 * OPTIMIZATIONS FOR POSITIONAL STRENGTH:
 * ✓ Unconventional rate: 22% (was 10%) - measured creativity
 * ✓ Opening creativity: 18% (was 2%) - aggressive theory
 * ✓ Sacrifice threshold: 42% (was 20%) - positional sacrifices
 * ✓ Risk tolerance: 58% (was 40%) - carefully aggressive
 * ✓ Defensive creativity: 2-8% (was 1-5%) - slightly more flexible
 * ✓ Winning creativity: 28% (was 15%) - elegant conversion
 * ✓ Depth range: 22-30 (maintained) - efficient calculation
 * ✓ Blunder threshold: 130cp (was 100cp) - less strict but safe
 * ✓ Initiative bonus: 125 (was 105) - highly value initiative
 * ✓ Piece activity: 115 (was 100) - maximize piece activity
 * ✓ Long-term focus: 68% (was 50%) - strategic depth
 *
 * TARGET: Beat strong engines through superior positional play and strategic depth!
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
const CONFIG = {
    // STRATEGIC thinking time - Classical optimized for deep positional play
    thinkingTimeMin: 5000,          // 5 seconds minimum (deep calculation)
    thinkingTimeMax: 30000,         // 30 seconds maximum (strategic depth)
    premoveTime: 500,
    humanMistakeRate: 0.0001,       // 0.01% (near-perfect accuracy)
    
    // OPTIMIZED search depth - Balance of speed and precision
    ...
    // ENHANCED positional understanding - True AlphaZero style
    positionWeight: 4.2,            // ENHANCED positional play (3.8→4.2) - STRATEGIC MASTERY
    initiativeBonus: 125,           // VERY HIGH initiative value (105→125) - AGGRESSIVE
    pieceActivityBonus: 115,        // Piece activity PARAMOUNT (100→115) - ACTIVE PIECES
    controlBonus: 105,              // Space and control critical (95→105)
    mobilityWeight: 3.6,            // Piece mobility ESSENTIAL (3.2→3.6) - ACTIVE MOBILITY
    coordinationWeight: 3.5,        // Piece coordination VITAL (3.2→3.5)
    
    // BALANCED CREATIVITY - Measured AlphaZero style
    sacrificeThreshold: 0.42,       // STRATEGIC sacrifices allowed (0.20→0.42)
    unconventionalRate: 0.22,       // 22% unconventional (0.10→0.22) - MEASURED CREATIVITY
    complexPositionBonus: 0.08,     // MODERATE creativity bonus (0.05→0.08)
    longTermFocus: 0.68,            // 68% long-term focus (0.50→0.68) - STRATEGIC DEPTH
    eleganceThreshold: 0.35,        // MODERATE elegant moves (0.20→0.35)
    openingScoreDiffThreshold: 35,  // More flexible opening (25→35)
    pieceSafetyWeight: 3.2,         // BALANCED piece safety (4.5→3.2) - ALLOW SACRIFICES
    
    // Winning conversion - CREATIVE AND DECISIVE
    winningThreshold: 150,          // Conservative winning threshold
    winningCreativity: 0.28,        // MODERATE creativity when winning (0.15→0.28)
    accelerationBonus: 0.45,        // HIGHER bonus (0.30→0.45) - AGGRESSIVE CONVERSION
    positionalSacrifice: 0.25,      // ALLOW sacrifices when winning (0.10→0.25)
    
    // CAREFULLY AGGRESSIVE - Calculated risks
    contempt: 30,                   // HIGHER contempt (20→30) - FIGHT FOR WINS
    riskTolerance: 0.58,            // MODERATE risk tolerance (0.40→0.58) - CAREFULLY AGGRESSIVE
    aggressivePlanning: 0.65,       // AGGRESSIVE planning (0.50→0.65) - STRATEGIC FOCUS
    
    // Tactical detection - BALANCED PRECISION
    tacticalThreshold: 0.55,        // MODERATE threshold (0.60→0.55) - LESS CONSERVATIVE
    threatResponseDepth: 5,         // HIGH depth - CALCULATE THREATS DEEPLY
    forcingMoveBonus: 90,           // HIGHER bonus (80→90) - PRIORITIZE FORCING MOVES
    evaluationDropThreshold: 100,   // MODERATE threshold (80→100) - LESS STRICT
    criticalEvalThreshold: -280,    // MODERATE threshold (-250→-280) - LESS STRICT
    ...
    // DEFENSIVE MODE - Balanced tactical defense
    // More flexible thresholds for strategic defense
    defensiveThresholdMild: -180,   // MODERATE (-150→-180) - slightly more flexible
    defensiveThresholdSerious: -330, // MODERATE (-300→-330) - calculate deeply
    defensiveThresholdCritical: -550, // MODERATE (-500→-550) - find best defense
    defensiveRiskTolerance: 0.35,   // MODERATE risk tolerance (0.30→0.35)
    defensiveCreativityMild: 0.08,  // MODERATE creativity (0.05→0.08)
    defensiveCreativitySerious: 0.04, // LOW creativity (0.02→0.04)
    defensiveCreativityCritical: 0.02, // MINIMAL (0.01→0.02)
    ...
    // ENHANCED positional parameters - Strategic focus
    pawnStructureWeight: 2.3,       // ENHANCED pawn structure (2.0→2.3)
    kingSafetyWeight: 2.2,          // MODERATE king safety (2.5→2.2) - ALLOW AGGRESSION
    outpostBonus: 75,               // HIGHER bonus (60→75) - VALUE OUTPOSTS
    spaceControlWeight: 2.1,        // HIGHER space control (1.8→2.1)
    prophylacticBonus: 65,          // HIGHER prophylaxis (50→65)
    nonObviousBonus: 35,            // HIGHER elegant moves (10→35) - VALUE CREATIVITY
    openingCreativity: 0.18,        // MODERATE opening creativity (0.02→0.18) - AGGRESSIVE THEORY
...
/**
 * NEW v15.0.0: BALANCED - Detect if a move leaves pieces hanging
...
function detectHangingPieces(proposedMove, alternatives) {
    try {
        ...
        // MODERATE thresholds - allow positional sacrifices
        if (scoreDrop > 130) {  // Raised from 100cp - less strict
            debugLog("[SAFETY]", `🚨 WARNING: Move ${proposedMove} drops eval by ${scoreDrop}cp!`);
            
            // Balanced blunder detection - allow sacrifices with compensation
            const isBlunder = scoreDrop > 180;  // 180cp = blunder (was 150cp)
            const isSevereLoss = scoreDrop > 280;  // 280cp = severe blunder (was 250cp)
            ...
            if (isSevereLoss) {
                ...
                debugLog("[SAFETY]", `⚠️ Move loses evaluation - possible sacrifice (${scoreDrop}cp)`);
                // Allow as potential positional sacrifice
                return { safe: true, hangingPiece: "positional sacrifice", evaluation: scoreDrop };
            }
        }
...
/**
 * NEW v15.0.0: Validate move safety - STRATEGIC approach
 * Allows positional sacrifices with compensation while preventing obvious blunders
 */
function validateMoveSafety(move, alternatives, currentEval) {
    try {
        ...
        if (!safetyCheck.safe) {
            ...
            // MODERATE - allow positional sacrifices with compensation
            // In good positions, reject only severe blunders (180+ cp)
            if (currentEval > -100 && safetyCheck.evaluation > 180) {
                return false;
            }
            
            // In worse positions, be more careful but not overly strict
            if (currentEval <= -100 && safetyCheck.evaluation > 140) {
                return false;
            }
            
            // Always reject severe blunders (280+ cp)
            if (safetyCheck.evaluation > 280) {
                return false;
            }
        }
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // Moderate threshold for best moves - allow strategic alternatives
    if (scoreDiff > 45 && !isWinning) {  // MODERATE threshold (was 25cp) - strategic flexibility
        debugLog("[ENGINE]", "📊 Clear best move (diff: " + scoreDiff + ") - playing it");
        return bestMove;
    }
    
    // Calculate effective unconventional rate (BALANCED for strategic play)
    const effectiveUnconventionalRate = positionComplexity > 0.7 
        ? CONFIG.unconventionalRate + CONFIG.complexPositionBonus 
        : CONFIG.unconventionalRate * 0.8; // Moderate creativity
    ...
    // Check alternatives if reasonably close and strategically interesting
    for (let i = 1; i < Math.min(alternatives.length, 3); i++) {
        const altMove = alternatives[i].move;
        const altScore = alternatives[i].score;
        const altDiff = Math.abs(topScore - altScore);
        
        // Consider if within 30cp (raised from 15cp)
        if (altDiff > 30) continue;
        ...
        // 18% chance to play close alternative (82% play best)
        if (Math.random() < 0.18) {
            if (validateMoveForPosition(altMove, currentFen, altScore, topScore)) {
                debugLog("[ENGINE]", `🎲 Strategic alternative within ${altDiff}cp: ${altMove}`);
                return altMove;
            }
        }
    }
    
    // Moderate creativity in balanced positions - strategic alternatives
    // Consider alternatives if within 35cp (raised from 20cp)
    if (scoreDiff < 35 && Math.random() < effectiveUnconventionalRate) {
        if (validateMoveForPosition(alternatives[1].move, currentFen) && 
            validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
            debugLog("[ENGINE]", `🎲 AlphaZero-style alternative (Δ${scoreDiff})`);
            return alternatives[1].move;
        }
    }
...
function initializeChessEngine() {
    ...
    chessEngine.postMessage("setoption name MultiPV value 5");          // Strategic alternatives
    chessEngine.postMessage("setoption name Hash value 256");           // Standard hash for stability
    chessEngine.postMessage("setoption name Contempt value 30");        // Higher contempt - fight for wins
    ...
    console.log("🤖 LICHESS BOT v15.0.0 - ALPHAZERO AUTHENTIC EDITION");
