// ==UserScript==
// @name         Lichess Bot - TRUE ALPHAZERO v13.0.0 AUTHENTIC EDITION
// @description  Authentic AlphaZero - Aggressive, Creative, Positional Mastery
// @author       AlphaZero Ultimate Edition
// @version      13.0.0-AUTHENTIC-ALPHAZERO
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * AUTHENTIC ALPHAZERO BOT v13.0.0 - TRUE POSITIONAL MASTERY
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * NEW v13.0.0 - TRUE ALPHAZERO ESSENCE:
 * ⚔️ AUTHENTIC: MAXIMUM creativity (50%) - true self-taught genius
 * ⚔️ AUTHENTIC: AGGRESSIVE positional sacrifices (75%) - initiative over material
 * ⚔️ AUTHENTIC: HIGH contempt (90) - fights for wins with elegance
 * ⚔️ AUTHENTIC: MAXIMUM risk tolerance (95%) - fearless attacking spirit
 * ⚔️ AUTHENTIC: Aggressive opening theory (45% creativity) - Sicilian mastery
 * ⚔️ AUTHENTIC: Elegant moves prioritized (60% threshold) - non-obvious brilliance
 * 
 * TRUE ALPHAZERO CHARACTER v13.0.0:
 * ✨ FREE-PLAYING: No restrictions, pure positional creativity
 * ✨ POSITIONAL SACRIFICES: Trades material for initiative (75% willingness)
 * ✨ LONG-HORIZON: Deep strategic planning (3-5 moves ahead)
 * ✨ ELEGANT MOVES: Non-obvious, sophisticated alternatives (60% preference)
 * ✨ PIECE ACTIVITY: Coordination, mobility, space control paramount
 * ✨ AGGRESSIVE OPENING: Sicilian Dragon, open positions, fighting chess
 * ✨ CREATIVE ENDGAME: Active king play, centralization, advanced technique
 * ✨ INTUITIVE: Positional understanding over immediate tactics
 * ✨ FIGHTS CREATIVELY: Maintains style even when behind
 * ✨ DEEP EXPLORATION: Considers 3rd, 4th, 5th alternatives regularly
 * ✨ SUPERHUMAN DEPTH: 40-75 depth for strategic mastery
 * ✨ CONSISTENT STYLE: No evaluation-based style changes
 * 
 * REMOVED ALL RESTRICTIONS:
 * ❌ Conservative safety checks removed - trusts intuition
 * ❌ Defensive panic modes removed - consistent creativity
 * ❌ Passed pawn overreaction removed - positional trust
 * ❌ Tactical forcing removed - allows creative solutions
 * ❌ Opening rigidity removed - 45% creativity from move 1
 * ❌ Winning caution removed - accelerates with 70% creativity
 * 
 * PLAYING STYLE [ECHOING ORIGINAL ALPHAZERO]:
 * - POSITIONAL SACRIFICES: Initiative, activity, space over material
 * - LONG-TERM PLANNING: Deep strategic understanding, 3-5 move horizons
 * - AGGRESSIVE OPENINGS: Sicilian mastery, fighting for advantage
 * - ELEGANT DECISIONS: Non-obvious, sophisticated, beautiful moves
 * - INTUITIVE PLAY: Trusts positional metrics over raw evaluation
 * - CREATIVE ENDGAMES: King activity, centralization, perfect technique
 * - CONSISTENT STYLE: Same aggressive, creative play in all positions
 * - FIGHTING SPIRIT: Never accepts draws, fights for wins
 * 
 * Core Principles (Authentic AlphaZero):
 * ✓ MAXIMUM creativity (50% unconventional rate)
 * ✓ POSITIONAL sacrifices (75% willingness)
 * ✓ ELEGANT moves (60% preference threshold)
 * ✓ AGGRESSIVE openings (45% creativity)
 * ✓ LONG-TERM focus (98% strategic planning)
 * ✓ DEEP alternatives (3rd-5th line exploration)
 * ✓ HIGH contempt (90 - fighting for wins)
 * ✓ MAXIMUM risk (95% tolerance)
 * ✓ INTUITIVE decisions (trusts positional understanding)
 * ✓ CONSISTENT style (no defensive modes)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
const CONFIG = {
    // MAXIMUM thinking time - Classical optimized for deep strategic play
    ...
    // MAXIMUM search depth - True AlphaZero superhuman understanding
    baseDepth: 50,                  // Base depth - DEEPER CALCULATION
    strategicDepth: 65,             // Strategic positions - SUPERIOR PLANNING
    endgameDepth: 70,               // Endgame - PERFECT TECHNIQUE
    openingDepth: 55,               // Opening - AGGRESSIVE PREPARATION
    classicalDepth: 75,             // Classical - ABSOLUTE MAXIMUM DEPTH
    winningDepth: 65,               // Winning - DECISIVE CONVERSION
    tacticalDepth: 70,              // Tactical - TACTICAL MASTERY
    criticalDepth: 75,              // Critical - EMERGENCY MAXIMUM
    
    // Time management - optimized for superhuman play
    earlyGameSpeed: 1.5,            // More time in opening (aggressive prep)
    middleGameSpeed: 2.0,           // Maximum time in middlegame (strategic depth)
    endGameSpeed: 1.8,              // Extended endgame time (perfect technique)
    
    // AGGRESSIVE positional understanding - True AlphaZero style
    positionWeight: 3.8,            // MAXIMUM positional play (3.5→3.8)
    initiativeBonus: 105,           // HIGH initiative value (80→105) - AGGRESSIVE
    pieceActivityBonus: 100,        // Piece activity PARAMOUNT (85→100) - ACTIVE PIECES
    controlBonus: 95,               // Space and control critical (90→95)
    mobilityWeight: 3.2,            // Piece mobility ESSENTIAL (3.0→3.2) - ACTIVE MOBILITY
    coordinationWeight: 3.2,        // Piece coordination VITAL (3.0→3.2)
    
    // AUTHENTIC ALPHAZERO - Aggressive sacrifices and creativity
    sacrificeThreshold: 0.75,       // HIGH positional sacrifices (0.50→0.75) - INITIATIVE
    unconventionalRate: 0.50,       // 50% unconventional (0.25→0.50) - TRUE CREATIVITY
    complexPositionBonus: 0.40,     // HIGH creativity bonus (0.25→0.40) - COMPLEX POSITIONS
    longTermFocus: 0.98,            // 98% focus on long-term positional play
    eleganceThreshold: 0.60,        // HIGH favor for elegant moves (0.45→0.60) - BRILLIANCE
    openingScoreDiffThreshold: 80,  // Aggressive opening creativity (60→80)
    pieceSafetyWeight: 2.8,         // Balanced piece safety (4.0→2.8) - TRUSTS INTUITION
    
    // Winning conversion - AGGRESSIVE AND DECISIVE
    winningThreshold: 150,          // Conservative winning threshold
    winningCreativity: 0.70,        // HIGH creativity when winning (0.40→0.70) - DECISIVE
    accelerationBonus: 0.70,        // HIGH bonus (0.50→0.70) - ACCELERATE WINS
    positionalSacrifice: 0.75,      // AGGRESSIVE when winning (0.40→0.75) - ACTIVITY
    
    // AUTHENTIC ALPHAZERO PERSONALITY - Fighting Spirit
    contempt: 90,                   // HIGH contempt (30→90) - FIGHTS FOR WINS
    riskTolerance: 0.95,            // MAXIMUM risk tolerance (0.70→0.95) - FEARLESS
    aggressivePlanning: 0.98,       // AGGRESSIVE planning (0.85→0.98) - INITIATIVE
    
    // Tactical detection - BALANCED CALCULATION
    tacticalThreshold: 0.75,        // Balanced threshold (0.70→0.75) - TRUST INTUITION
    threatResponseDepth: 3,         // Moderate (4→3) - TRUST POSITIONAL PLAY
    forcingMoveBonus: 40,           // MODERATE (50→40) - ALLOW CREATIVITY
    evaluationDropThreshold: 150,   // MODERATE (100→150) - TRUST POSITION
    criticalEvalThreshold: -400,    // MODERATE (-300→-400) - FIGHT CREATIVELY
    
    // Anti-draw and repetition - Fight for wins
    repetitionPenalty: 120,         // STRONG penalty for repetitions (100→120)
    antiDrawBias: 0.98,             // MAXIMUM bias against draws (0.95→0.98)
    repetitionHistoryDepth: 12,     // Track more positions
    
    // DEFENSIVE MODE - AlphaZero maintains creativity when behind
    // These thresholds are lenient, maintaining aggressive creative style
    defensiveThresholdMild: -400,   // LENIENT - maintain creativity
    defensiveThresholdSerious: -650, // LENIENT - still creative
    defensiveThresholdCritical: -900, // EXTREME - virtually never panic
    defensiveRiskTolerance: 0.85,   // HIGH risk tolerance when behind (0.70→0.85)
    defensiveCreativityMild: 0.50,  // HIGH creativity when behind (0.40→0.50)
    defensiveCreativitySerious: 0.45, // MAINTAIN creativity in trouble (0.30→0.45)
    defensiveCreativityCritical: 0.40, // ALWAYS creative (0.20→0.40)
    defensiveDepthBonus: 1,         // MINIMAL extra depth when defending (2→1)
    
    // TRUST POSITIONAL UNDERSTANDING - Minimal passed pawn panic
    passedPawnDangerRank: 1,        // Only promotion squares (rank 1/8) (2→1)
    passedPawnDepthBonus: 0,        // NO bonus - trust position (1→0)
    
    // AGGRESSIVE positional parameters - True AlphaZero style
    pawnStructureWeight: 2.5,       // HIGHER pawn structure importance (2.2→2.5)
    kingSafetyWeight: 1.6,          // MODERATE king safety (2.0→1.6) - AGGRESSIVE
    outpostBonus: 85,               // HIGHER bonus for piece outposts (70→85)
    spaceControlWeight: 2.3,        // HIGHER space control (2.0→2.3)
    prophylacticBonus: 75,          // HIGHER prophylaxis (60→75)
    nonObviousBonus: 65,            // HIGH elegant moves (40→65) - FAVOR BRILLIANCE
    openingCreativity: 0.45,        // HIGH opening creativity (0.15→0.45) - AGGRESSIVE
...
/**
 * NEW v13.0.0: BALANCED - Detect if a move leaves pieces hanging
 * Allows positional sacrifices while preventing obvious blunders
...
function detectHangingPieces(proposedMove, alternatives) {
    try {
        ...
        // v13.0.0: BALANCED thresholds - allow positional sacrifices
        // Only flag serious blunders, trust AlphaZero's intuition
        if (scoreDrop > 300) {  // INCREASED from 150 - allow sacrifices
            debugLog("[SAFETY]", `🚨 WARNING: Move ${proposedMove} drops eval by ${scoreDrop}cp!`);
            
            // Balanced blunder detection
            const isBlunder = scoreDrop > 400;  // INCREASED from 200
            const isSevereLoss = scoreDrop > 550;  // INCREASED from 300
            ...
            if (isSevereLoss) {
                ...
                debugLog("[SAFETY]", `⚠️ WARNING: Significant blunder detected (${scoreDrop}cp)`);
                return { safe: false, hangingPiece: "piece", evaluation: scoreDrop };
            } else {
                debugLog("[SAFETY]", `💎 Positional sacrifice considered (${scoreDrop}cp)`);
                return { safe: true, hangingPiece: "positional", evaluation: scoreDrop };
            }
        }
        
        // v13.0.0: Allow positional sacrifices (80-300cp)
        if (scoreDrop > 80 && scoreDrop <= 300) {
            debugLog("[SAFETY]", `💎 Positional sacrifice range: ${scoreDrop}cp - ALLOWED`);
            return { safe: true, hangingPiece: null, evaluation: scoreDrop };
        }
...
/**
 * NEW v13.0.0: Validate move safety - BALANCED approach
 * Allows positional sacrifices while preventing obvious blunders
 */
function validateMoveSafety(move, alternatives, currentEval) {
    try {
        ...
        if (!safetyCheck.safe) {
            ...
            // v13.0.0: BALANCED - allow positional sacrifices, reject true blunders
            // In good positions, allow sacrifices up to 300cp for initiative
            if (currentEval > -100 && safetyCheck.evaluation > 400) {
                return false; // Only reject severe blunders
            }
            
            // In slightly worse positions, allow fighting sacrifices
            if (currentEval > -300 && safetyCheck.evaluation > 450) {
                return false; // Reject major blunders
            }
            
            // Even in bad positions, reject catastrophic blunders only
            if (safetyCheck.evaluation > 550) {
                return false; // Severe material loss
            }
        }
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // AGGRESSIVE opening play (first 15 moves) - v13.0.0
    if (moveCount <= 15) {
        debugLog("[ENGINE]", `📖 OPENING PHASE (move ${moveCount}) - AGGRESSIVE CREATIVITY!`);
        ...
        // Apply HIGH opening creativity (45% - AGGRESSIVE) v13.0.0
        // Favor fighting, creative moves from move 1
        if (Math.random() > CONFIG.openingCreativity) {
            debugLog("[ENGINE]", "📖 OPENING: Best fighting move");
            return bestMove;
        }
        
        // Allow creative alternatives in opening
        if (scoreDiff < 50 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            if (validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
                // Check if alternative is interesting (activity, coordination)
                const coordination = evaluatePieceCoordination(currentFen);
                const activity = evaluatePieceActivity(currentFen);
                
                if (coordination > 0.50 || activity > 0.55 || scoreDiff < 25) {
                    debugLog("[ENGINE]", `📖 OPENING: Creative aggressive alternative (Δ${scoreDiff})`);
                    return alternatives[1].move;
                }
            }
        }
        
        debugLog("[ENGINE]", "📖 OPENING: Best aggressive move");
        return bestMove;
    }
    ...
    // ═══════════════════════════════════════════════════════════
    // AUTHENTIC ALPHAZERO: HIGH creativity even when behind
    // AlphaZero fights with style, not panic
    // ═══════════════════════════════════════════════════════════
    ...
    if (isBehind) {
        ...
        if (isLosing) {
            ...
            debugLog("[ENGINE]", `🎭 LOSING (${topScore}cp) - Fighting with AlphaZero creativity!`);
        } else if (isFarBehind) {
            creativityLevel = CONFIG.defensiveCreativitySerious;
            positionStatus = "FAR BEHIND";
            debugLog("[ENGINE]", `🎭 FAR BEHIND (${topScore}cp) - Creative fighting chess!`);
        } else {
            debugLog("[ENGINE]", `🎭 BEHIND (${topScore}cp) - AlphaZero fights with style!`);
        }
        
        // AUTHENTIC ALPHAZERO: Fight creatively even when losing
        // Only force best move in extreme situations
        if (isLosing && scoreDiff > 120 && topScore < -850) {
            debugLog("[ENGINE]", "🎭 Extreme position - best practical chance");
            return bestMove;
        }
        ...
        // Apply HIGH creative fight-back style (50%+ creativity)
        if (Math.random() > creativityLevel) {
            debugLog("[ENGINE]", `🎭 ${positionStatus}: Best fighting move`);
            return bestMove;
        }
        
        // Look for aggressive counterplay alternatives
        if (scoreDiff < 50 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            ...
            // More willing to sacrifice for activity when behind
            if (coordination > 0.50 || activity > 0.55 || mobility > 0.55 || scoreDiff < 25) {
                debugLog("[ENGINE]", `🎭 ${positionStatus}: Aggressive counterplay! (Δ${scoreDiff})`);
                return alternatives[1].move;
            }
        }
        
        debugLog("[ENGINE]", `🎭 ${positionStatus}: Creative fighting move`);
        return bestMove;
    }
    ...
    // v13.0.0: Balanced score difference threshold - allow creativity
    if (scoreDiff > 70 && !isWinning) {  // INCREASED from 40 to 70 - more creativity
        debugLog("[ENGINE]", "📊 Clear best move (diff: " + scoreDiff + ") - playing it");
        return bestMove;
    }
    
    // Calculate effective unconventional rate (HIGH CREATIVITY for authentic style)
    const effectiveUnconventionalRate = positionComplexity > 0.7 
        ? CONFIG.unconventionalRate + CONFIG.complexPositionBonus 
        : CONFIG.unconventionalRate * 0.85; // HIGHER creativity in simple positions (0.70→0.85)
    ...
    // AUTHENTIC ALPHAZERO: MAXIMUM preference for prophylactic and non-obvious moves
    for (let i = 1; i < Math.min(alternatives.length, 5); i++) {
        const altMove = alternatives[i].move;
        const altScore = alternatives[i].score;
        const altDiff = Math.abs(topScore - altScore);
        
        // Check safety (allows sacrifices)
        if (!validateMoveSafety(altMove, alternatives, currentEval)) {
            continue; // Skip true blunders only
        }
        
        // Prophylactic move bonus - HIGH PRIORITY
        if (isProphylacticMove(altMove, currentFen, alternatives) && altDiff < 70) {
            debugLog("[ENGINE]", `🛡️ ALPHAZERO: Prophylactic move detected: ${altMove} (Δ${altDiff})`);
            if (Math.random() < 0.75) { // 75% chance to play prophylactic move
                return altMove;
            }
        }
        
        // Non-obvious move bonus - AUTHENTIC ALPHAZERO
        if (isNonObviousMove(altMove, alternatives, positionComplexity) && altDiff < 80) {
            if (positionalScore > 0.45) { // Lenient threshold
                debugLog("[ENGINE]", `💫 ALPHAZERO: Non-obvious elegant move: ${altMove} (Δ${altDiff})`);
                if (Math.random() < 0.70) { // 70% chance for non-obvious moves
                    return altMove;
                }
            }
        }
    ...
    // v13.0.0: HIGH creativity in complex and balanced positions
    if (positionComplexity > 0.55 && scoreDiff < 60 && Math.random() < effectiveUnconventionalRate) {  // INCREASED creativity
        ...
        if (validateMoveForPosition(alternatives[1].move, currentFen) && 
            ...
            // AUTHENTIC ALPHAZERO: Prioritize positional improvement
            ...
            if (needsImprovement && scoreDiff < 60) {
            ...
            // AUTHENTIC ALPHAZERO: Space control and initiative - HIGH priority
            if ((mobility < 0.65 || spaceControl < 0.55 || activity < 0.70) && scoreDiff < 60) {
                debugLog("[ENGINE]", `🌊 ALPHAZERO: Initiative/Activity improvement (Δ${scoreDiff})`);
                return alternatives[1].move;
            }
            
            // AUTHENTIC ALPHAZERO: Pawn structure and outposts
            if ((pawnStructure < 0.50 || outposts < 0.45) && scoreDiff < 55) {
                debugLog("[ENGINE]", `♟️ ALPHAZERO: Structural improvement (Δ${scoreDiff})`);
                return alternatives[1].move;
            }
            
            // King safety only in early middlegame when necessary
            if (gamePhase === "middlegame" && moveCount < 25 && kingSafety < 0.35 && scoreDiff < 50) {
                debugLog("[ENGINE]", `👑 ALPHAZERO: King safety improvement (Δ${scoreDiff})`);
                return alternatives[1].move;
            }
        }
    }
    
    // AUTHENTIC ALPHAZERO: Deep lines exploration (MAXIMUM creativity)
    if (alternatives.length > 2 && positionComplexity > 0.60) {
        const scoreDiff2 = Math.abs(topScore - alternatives[2].score);
        
        // HIGHLY willing to explore deep lines
        if (scoreDiff2 < 60 && Math.random() < (effectiveUnconventionalRate * 0.75)) {
        ...
        // AUTHENTIC ALPHAZERO: Consider 4th line in complex positions
        if (alternatives.length > 3 && positionComplexity > 0.70) {
            const scoreDiff3 = Math.abs(topScore - alternatives[3].score);
            if (scoreDiff3 < 50 && Math.random() < (effectiveUnconventionalRate * 0.55)) {
        ...
        // AUTHENTIC ALPHAZERO: Consider 5th line in highly complex positions
        if (alternatives.length > 4 && positionComplexity > 0.80) {
            const scoreDiff4 = Math.abs(topScore - alternatives[4].score);
            if (scoreDiff4 < 45 && Math.random() < (effectiveUnconventionalRate * 0.35)) {
                if (validateMoveForPosition(alternatives[4].move, currentFen) &&
                    validateMoveSafety(alternatives[4].move, alternatives, currentEval)) {
                    debugLog("[ENGINE]", `🎆 ALPHAZERO: 5th-line self-taught brilliance (Δ${scoreDiff4})`);
