// ==UserScript==
// @name         Lichess Bot - MAXIMUM STRENGTH v14.0.0
// @description  Optimized for Strength - Tactical Precision, Solid Play, Beats AI Level 8
// @author       Strength Focused Edition
// @version      14.0.0-STRENGTH-OPTIMIZED
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * STRENGTH OPTIMIZED BOT v14.0.0 - MAXIMUM PLAYING STRENGTH
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * NEW v14.0.0 - STRENGTH FOCUSED APPROACH:
 * 💪 TACTICAL PRECISION: 95%+ engine best moves - minimal creativity
 * 💪 BLUNDER PREVENTION: Strict 100cp threshold - no hanging pieces
 * 💪 SOLID OPENINGS: 98% book moves - play proven theory
 * 💪 ACCURATE DEFENSE: Engine best moves when behind - no panic
 * 💪 SAFE CONVERSION: Maintain advantage when winning - solid technique
 * 💪 EFFICIENT DEPTH: Optimized 22-30 depth - fast tactical precision
 * 
 * CORE PLAYING PRINCIPLES v14.0.0:
 * ✅ BEST MOVES FIRST: 95% play engine's top choice
 * ✅ NO BLUNDERS: Strict move validation, reject 100cp+ drops
 * ✅ TACTICAL AWARENESS: Deep threat detection (depth +5 when needed)
 * ✅ SOLID DEFENSE: Calculate accurately when behind
 * ✅ SAFE WINS: Convert advantages without risks
 * ✅ BOOK KNOWLEDGE: Follow opening theory consistently
 * ✅ KING SAFETY: Prioritize solid king position (weight 2.5)
 * ✅ PIECE SAFETY: Maximum blunder prevention (weight 4.5)
 * 
 * OPTIMIZATIONS FOR STRENGTH:
 * ✓ Unconventional rate: 10% (was 50%) - prioritize best moves
 * ✓ Opening creativity: 2% (was 45%) - follow theory
 * ✓ Sacrifice threshold: 20% (was 75%) - concrete play
 * ✓ Risk tolerance: 40% (was 95%) - safe approach
 * ✓ Defensive creativity: 1-5% (was 40-50%) - engine best when behind
 * ✓ Winning creativity: 15% (was 70%) - safe conversion
 * ✓ Depth range: 22-30 (was 50-75) - efficient calculation
 * ✓ Blunder threshold: 100cp (was 300cp+) - strict prevention
 * ✓ King safety: 2.5 (was 1.6) - solid foundation
 * ✓ Piece safety: 4.5 (was 2.8) - prevent material loss
 *
 * TARGET: Beat Lichess AI Level 8 consistently with solid, strong chess!
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
const CONFIG = {
    ...
    // MAXIMUM search depth - Optimized for strength
    baseDepth: 22,                  // Base depth - SOLID CALCULATION
    strategicDepth: 26,             // Strategic positions - DEEP PLANNING
    endgameDepth: 28,               // Endgame - PERFECT TECHNIQUE
    openingDepth: 20,               // Opening - FAST THEORY
    classicalDepth: 30,             // Classical - MAXIMUM DEPTH
    winningDepth: 24,               // Winning - DECISIVE CONVERSION
    tacticalDepth: 28,              // Tactical - TACTICAL PRECISION
    criticalDepth: 30,              // Critical - EMERGENCY MAXIMUM
    ...
    // STRENGTH FOCUSED - Prioritize best moves
    sacrificeThreshold: 0.20,       // LOW positional sacrifices - CONCRETE PLAY
    unconventionalRate: 0.10,       // 10% unconventional - PRIORITIZE BEST MOVES
    complexPositionBonus: 0.05,     // LOW creativity bonus - STRENGTH FIRST
    longTermFocus: 0.50,            // 50% focus on long-term, 50% tactical
    eleganceThreshold: 0.20,        // LOW favor for elegant moves - BEST MOVES FIRST
    openingScoreDiffThreshold: 25,  // Strict opening - play best moves
    pieceSafetyWeight: 4.5,         // HIGH piece safety - PREVENT BLUNDERS
    
    // Winning conversion - SOLID AND DECISIVE
    winningThreshold: 150,          // Conservative winning threshold
    winningCreativity: 0.15,        // LOW creativity when winning - CONVERT SAFELY
    accelerationBonus: 0.30,        // MODERATE bonus - SOLID CONVERSION
    positionalSacrifice: 0.10,      // MINIMAL when winning - KEEP ADVANTAGE
    
    // STRENGTH FOCUSED - Solid play
    contempt: 20,                   // MODERATE contempt - SOLID PLAY
    riskTolerance: 0.40,            // LOW risk tolerance - SAFE PLAY
    aggressivePlanning: 0.50,       // BALANCED planning - TACTICAL + POSITIONAL
    
    // Tactical detection - MAXIMUM PRECISION
    tacticalThreshold: 0.60,        // LOW threshold - DETECT TACTICS EARLY
    threatResponseDepth: 5,         // HIGH depth - CALCULATE THREATS DEEPLY
    forcingMoveBonus: 80,           // HIGH bonus - PRIORITIZE FORCING MOVES
    evaluationDropThreshold: 80,    // LOW threshold - DETECT PROBLEMS EARLY
    criticalEvalThreshold: -250,    // STRICT threshold - DEFEND ACCURATELY
    ...
    // DEFENSIVE MODE - Strong tactical defense
    // Strict thresholds for accurate defensive play
    defensiveThresholdMild: -150,   // STRICT - defend early
    defensiveThresholdSerious: -300, // STRICT - calculate accurately
    defensiveThresholdCritical: -500, // STRICT - find best defense
    defensiveRiskTolerance: 0.30,   // LOW risk tolerance when behind - SOLID DEFENSE
    defensiveCreativityMild: 0.05,  // MINIMAL creativity when behind - BEST MOVES
    defensiveCreativitySerious: 0.02, // ALMOST NONE in trouble - ENGINE BEST
    defensiveCreativityCritical: 0.01, // VIRTUALLY NONE - PURE ENGINE
    defensiveDepthBonus: 3,         // HIGH extra depth when defending - ACCURACY
    ...
    // SOLID positional parameters - Strength focused
    pawnStructureWeight: 2.0,       // BALANCED pawn structure importance
    kingSafetyWeight: 2.5,          // HIGH king safety - SOLID FOUNDATION
    outpostBonus: 60,               // MODERATE bonus for piece outposts
    spaceControlWeight: 1.8,        // MODERATE space control
    prophylacticBonus: 50,          // MODERATE prophylaxis
    nonObviousBonus: 10,            // LOW elegant moves - BEST MOVES ONLY
    openingCreativity: 0.02,        // MINIMAL opening creativity - PLAY THEORY
...
function detectHangingPieces(proposedMove, alternatives) {
    try {
        ...
        // STRICT thresholds - prevent blunders
        if (scoreDrop > 100) {  // Detect any significant drop
            debugLog("[SAFETY]", `🚨 WARNING: Move ${proposedMove} drops eval by ${scoreDrop}cp!`);
            
            // Strict blunder detection
            const isBlunder = scoreDrop > 150;  // 150cp = blunder
            const isSevereLoss = scoreDrop > 250;  // 250cp = severe blunder
            ...
            if (isSevereLoss) {
                ...
                debugLog("[SAFETY]", `⚠️ Move loses significant evaluation (${scoreDrop}cp)`);
                return { safe: false, hangingPiece: "material", evaluation: scoreDrop };
            }
        }
...
function validateMoveSafety(move, alternatives, currentEval) {
    try {
        ...
        if (!safetyCheck.safe) {
            ...
            // STRICT - prevent all significant blunders
            // In good positions, reject moves that drop 150+ cp
            if (currentEval > -100 && safetyCheck.evaluation > 150) {
                return false;
            }
            
            // In worse positions, be even more careful
            if (currentEval <= -100 && safetyCheck.evaluation > 100) {
                return false;
            }
            
            // Always reject severe blunders
            if (safetyCheck.evaluation > 200) {
                return false;
            }
        }
...
function getAlphaZeroBookMove(fen, activeColor) {
    ...
    if (moveCount <= 20) {
        // Use weighted selection based on book
    ...
    debugLog("[ENGINE]", `📖 BOOK MOVE (default): ${moves[0].name} - ${moves[0].move}`);
    return moves[0].move;
}
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // SOLID opening play (first 15 moves) - PLAY BEST MOVES
    if (moveCount <= 15) {
        debugLog("[ENGINE]", `📖 OPENING PHASE (move ${moveCount}) - PLAYING BEST THEORY`);
        
        // In opening, always play best move unless extremely close
        if (scoreDiff > 15) {
            debugLog("[ENGINE]", `📖 OPENING: Best move (Δ${scoreDiff})`);
            return bestMove;
        }
        
        // 98% chance to play best move in opening (only 2% for alternatives)
        if (Math.random() > 0.02) {
            debugLog("[ENGINE]", "📖 OPENING: Engine best move");
            return bestMove;
        }
        
        // Extremely rare: allow alternative only if within 10cp and safe
        if (scoreDiff < 10 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            if (validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
                debugLog("[ENGINE]", `📖 OPENING: Alternative within 10cp (Δ${scoreDiff})`);
    ...
    if (isBehind) {
        ...
        if (isLosing) {
            ...
            debugLog("[ENGINE]", `🛡️ LOSING (${topScore}cp) - Playing best defense`);
        } else if (isFarBehind) {
            creativityLevel = CONFIG.defensiveCreativitySerious;
            positionStatus = "FAR BEHIND";
            debugLog("[ENGINE]", `🛡️ FAR BEHIND (${topScore}cp) - Accurate defense`);
        } else {
            debugLog("[ENGINE]", `🛡️ BEHIND (${topScore}cp) - Playing best moves`);
        }
        
        // When behind, prioritize engine's best moves (minimal creativity)
        if (scoreDiff > 20) {
            debugLog("[ENGINE]", `🛡️ ${positionStatus}: Best move clearly superior (Δ${scoreDiff})`);
            return bestMove;
        }
        
        // 95-99% chance to play best move when behind (very minimal creativity)
        if (Math.random() > creativityLevel) {
            debugLog("[ENGINE]", `🛡️ ${positionStatus}: Engine best move`);
            return bestMove;
        }
        
        // Extremely rare alternative (only if within 10cp and safe)
        if (scoreDiff < 10 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            if (validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
                debugLog("[ENGINE]", `🛡️ ${positionStatus}: Alternative within 10cp (Δ${scoreDiff})`);
                return alternatives[1].move;
            }
        }
    ...
    if (isWinning && alternatives.length >= 2) {
        debugLog("[ENGINE]", `🏆 WINNING MODE (${topScore}cp) - Solid conversion`);
        ...
        // When winning, only consider alternatives if they keep significant advantage
        if (scoreDiff > 40) {
            debugLog("[ENGINE]", `🏆 WINNING: Best move significantly better (Δ${scoreDiff})`);
            return bestMove;
        }
        
        // 85% chance to play best move when winning (maintain advantage)
        if (Math.random() > CONFIG.winningCreativity) {
            debugLog("[ENGINE]", "🏆 WINNING: Playing best move");
            return bestMove;
        }
        
        // Consider alternative only if still clearly winning and within 30cp
        if (secondScore > 100 && scoreDiff < 30) {
            if (validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
                if (validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
                    debugLog("[ENGINE]", `🏆 WINNING: Safe alternative (${secondScore}cp, Δ${scoreDiff})`);
                    return alternatives[1].move;
                }
            }
        }
        
        // If crushing, focus on best conversion
        if (isCrushing) {
            ...
            debugLog("[ENGINE]", "⚡ Crushing advantage - best conversion");
            return bestMove;
        }
    }
    ...
    // Strict threshold for best moves - prioritize strength
    if (scoreDiff > 25 && !isWinning) {  // STRICT threshold - play best moves
        debugLog("[ENGINE]", "📊 Clear best move (diff: " + scoreDiff + ") - playing it");
        return bestMove;
    }
    
    // Calculate effective unconventional rate (LOW for strength)
    const effectiveUnconventionalRate = positionComplexity > 0.7 
        ? CONFIG.unconventionalRate + CONFIG.complexPositionBonus 
        : CONFIG.unconventionalRate * 0.5; // Very low creativity
    ...
    // Check alternatives only if extremely close and safe
    for (let i = 1; i < Math.min(alternatives.length, 3); i++) {
        const altMove = alternatives[i].move;
        const altScore = alternatives[i].score;
        const altDiff = Math.abs(topScore - altScore);
        
        // Only consider if within 15cp and safe
        if (altDiff > 15) continue;
        ...
        // 5% chance to play close alternative (95% play best)
        if (Math.random() < 0.05) {
            if (validateMoveForPosition(altMove, currentFen, altScore, topScore)) {
                debugLog("[ENGINE]", `🎲 Close alternative within ${altDiff}cp: ${altMove}`);
                return altMove;
            }
        }
    }
    
    // Minimal creativity in balanced positions - prioritize best moves
    // Only consider alternatives if within 20cp
    if (scoreDiff < 20 && Math.random() < effectiveUnconventionalRate) {
        if (validateMoveForPosition(alternatives[1].move, currentFen) && 
            validateMoveSafety(alternatives[1].move, alternatives, currentEval)) {
            debugLog("[ENGINE]", `🎲 Alternative within 20cp (Δ${scoreDiff})`);
            return alternatives[1].move;
        }
    }
...
function initializeChessEngine() {
    ...
    chessEngine.postMessage("setoption name MultiPV value 5");          // Focused on best moves
    chessEngine.postMessage("setoption name Hash value 256");           // Standard hash for stability
    chessEngine.postMessage("setoption name Contempt value 10");        // Low contempt for accuracy
    chessEngine.postMessage("setoption name Move Overhead value 30");   // Standard overhead
    chessEngine.postMessage("setoption name Skill Level value 20");     // Maximum skill
    chessEngine.postMessage("setoption name Threads value 2");          // Standard threads
    chessEngine.postMessage("setoption name UCI_LimitStrength value false"); // No strength limit
    chessEngine.postMessage("setoption name Minimum Thinking Time value 1000"); // 1s minimum
    chessEngine.postMessage("isready");
    
    console.log("🤖 LICHESS BOT v14.0.0 - MAXIMUM STRENGTH EDITION");
    console.log("💪 NEW v14.0.0: Optimized depth (22-30) - Efficient tactical precision");
    console.log("🛡️ NEW v14.0.0: STRICT blunder prevention - No hanging pieces!");
    console.log("🎯 NEW v14.0.0: 95%+ best moves - Minimal creativity, maximum strength");
    console.log("📚 NEW v14.0.0: Solid opening theory - Play best book moves");
    console.log("⚡ NEW v14.0.0: Tactical mastery - Deep threat detection (depth +5)");
    console.log("🔒 NEW v14.0.0: Enhanced safety - 100cp blunder threshold");
    console.log("✅ OPTIMIZED: MultiPV 5 - Focused calculation");
    console.log("✅ OPTIMIZED: King safety 2.5 - Solid defensive foundation");
    console.log("✅ OPTIMIZED: Piece safety 4.5 - Maximum blunder prevention");
    console.log("✅ OPTIMIZED: Winning conversion - Safe technique, no risks");
    console.log("✅ OPTIMIZED: Defensive accuracy - Engine best moves when behind");
    console.log("✅ OPTIMIZED: Opening discipline - 98% book moves, 2% creativity");
    console.log("🏆 MISSION: BEAT LICHESS AI LEVEL 8 WITH SOLID, STRONG CHESS!");
}
