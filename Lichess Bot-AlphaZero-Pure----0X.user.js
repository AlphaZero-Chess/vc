// ==UserScript==
// @name         Lichess Bot - PURE ALPHAZERO v5.1.0 DEADLOCK-PROOF EDITION
// @description  TRUE AlphaZero Echo - NEVER Stops Moving, Unconditional Recovery, Zero Deadlocks
// @author       Enhanced Human AI
// @version      5.1.0-ALPHAZERO-DEADLOCK-PROOF
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * PURE ALPHAZERO BOT v5.1.0 - DEADLOCK-PROOF EDITION
 * ═══════════════════════════════════════════════════════════════════════════
 * 
...
const CONFIG = {
    ...
    endgameDepth: 30,           // INCREASED: Endgame depth for perfect conversion (was 28)
    openingDepth: 22,           // INCREASED: Solid opening depth to avoid blunders
    classicalDepth: 32,         // INCREASED: Extra depth for classical time controls (was 30)
    winningDepth: 28,           // INCREASED: Depth when winning to find fastest mate/conversion (was 26)
    ...
    sacrificeThreshold: 0.50,   // INCREASED: Much more willing to sacrifice for position/activity (was 0.45)
    ...
    longTermFocus: 0.97,        // INCREASED: 97% focus on long-term play - deepest vision (was 0.95)
    eleganceThreshold: 0.40,    // INCREASED: Favor elegant moves more often (was 0.35)
    
    // NEW: Winning conversion parameters (ULTRA-SHARP finishes)
    winningThreshold: 80,       // LOWERED: Eval > +80cp = winning position - trigger even earlier! (was 100)
    winningCreativity: 0.65,    // INCREASED: 65% creativity when winning - ultra-maximum aggression! (was 0.55)
    accelerationBonus: 0.40,    // INCREASED: Bigger bonus for moves that increase advantage (was 0.35)
    positionalSacrifice: 0.55,  // INCREASED: Much more willing to sacrifice when winning (was 0.45)
    
    // AlphaZero personality - ULTRA-MAXIMUM EXPLOSIVE & AGGRESSIVE
    contempt: 120,              // INCREASED: Ultra-maximum contempt to avoid draws - ABSOLUTELY NO DRAWS! (was 95)
    riskTolerance: 0.82,        // INCREASED: Ultra-high risk tolerance for positional gains (was 0.78)
    aggressivePlanning: 0.95,   // INCREASED: Ultra-maximum aggressive long-term planning weight (was 0.92)
...
function getStrategicDepth(phase, isStrategic, timeLeft) {
    ...
    if (phase === "endgame" && timeLeft > 30000) {
        depth = Math.min(depth + 3, CONFIG.classicalDepth); // Increased from +2
...
/**
 * AlphaZero move selection - EXPLOSIVE CREATIVITY & SHARP CONVERSION & ANTI-DRAW
 */
...
function initializeChessEngine() {
    ...
    chessEngine.postMessage("setoption name MultiPV value 7");         // See more options for creativity
    chessEngine.postMessage("setoption name Hash value 512");          // Increased from 256
    chessEngine.postMessage("setoption name Contempt value 120");      // ULTRA-INCREASED: 120 for ABSOLUTELY NO DRAWS! (was 95)
    ...
    console.log("🤖 Pure AlphaZero v5.0.0 ULTIMATE EDITION - TRUE ECHO initialized");
    console.log("✅ REVOLUTIONARY: Anti-draw system - detects and avoids repetitions");
    console.log("✅ REVOLUTIONARY: Enhanced endgame - king activity, pawn races, perfect conversion");
    console.log("✅ ULTRA-AGGRESSIVE: Winning mode at +80cp (was +100cp), creativity 65% (was 55%)");
    console.log("✅ MAXIMUM CONTEMPT: 120 (was 95) - ABSOLUTELY NO DRAWS vs weak opponents!");
    console.log("✅ SUPERIOR TECHNIQUE: Depth 30-32, pawn race detection, promotion prevention");
    console.log("✅ ADAPTIVE DEFENSE: Solid play when behind, depth boost for accuracy");
    console.log("✅ TRUE ALPHAZERO: Explosive creativity, decisive play, perfect endgames");
}
...
console.log(`
═══════════════════════════════════════════════════════════════
🔥 PURE ALPHAZERO v5.1.0 - DEADLOCK-PROOF EDITION! 🔥
⚡ LOCK-FREE SYSTEM: NEVER STOPS MOVING ⚡
💎 ABSOLUTE SAFETY: UNCONDITIONAL RECOVERY 💎
═══════════════════════════════════════════════════════════════

CRITICAL FIX v5.1.0 (DEADLOCK-PROOF REWRITE):
🛡️ UNCONDITIONAL WATCHDOG: Overrides all conditions after timeout
🛡️ PER-COLOR TRACKING: White/Black independent state management
🛡️ ABSOLUTE RECOVERY: Health checks every 2 seconds
🛡️ FORCE UNLOCK: Hard reset when stuck conditions detected
🛡️ PROGRESSIVE ESCALATION: Multiple safety layers
🛡️ ZERO DEADLOCKS: Impossible to enter permanent stuck state

PROBLEM SOLVED: Bot stopped moving in game OL9OZhJD (move 56)
SOLUTION: Complete lock-free rewrite with unconditional escape paths

MAJOR ENHANCEMENTS v5.0.0 (EXPLOSIVE CREATIVITY):
