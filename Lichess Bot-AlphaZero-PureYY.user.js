// ==UserScript==
// @name         Lichess Bot - AUTHENTIC ALPHAZERO v10.0.0 SOLID GENIUS
// @description  Authentic AlphaZero - Tactically Solid Positional Genius
// @author       AlphaZero Authentic Edition
// @version      10.0.0-ALPHAZERO-SOLID
// @match         *://lichess.org/*
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * AUTHENTIC ALPHAZERO BOT v10.0.0 - SOLID GENIUS EDITION
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * NEW v10.0.0 - TACTICAL SOLIDITY ENHANCEMENTS:
 * 🛡️ CRITICAL: Piece safety validation - prevents hanging pieces (Queen blunder fix)
 * 🛡️ ENHANCED: Deeper search (40-60 depth) for perfect calculation
 * 🛡️ ENHANCED: Strengthened all positional weights (mobility, coordination, activity)
 * 🛡️ ENHANCED: Better threat detection and evaluation tracking
 * 🛡️ ENHANCED: Safety checks on ALL creative alternatives
 * 
 * AUTHENTIC v10.0.0 - TRUE ALPHAZERO CHARACTER:
...
const CONFIG = {
    ...
    // SUPERHUMAN search depth - MASSIVELY INCREASED for tactical solidity
    baseDepth: 40,                  // Base depth (35→40) - DEEPER CALCULATION
    strategicDepth: 55,             // Strategic positions (45→55) - SUPERHUMAN PLANNING
    endgameDepth: 55,               // Endgame (48→55) - PERFECT TECHNIQUE
    openingDepth: 40,               // Opening (35→40) - GRANDMASTER PREP
    classicalDepth: 60,             // Classical (50→60) - MAXIMUM DEPTH
    winningDepth: 52,               // Winning (45→52) - PERFECT CONVERSION
    tacticalDepth: 58,              // Tactical (48→58) - TACTICAL MASTERY (prevent blunders)
    criticalDepth: 60,              // Critical (50→60) - EMERGENCY DEPTH
    ...
    // SUPERHUMAN positional understanding (TRUE ALPHAZERO) - STRENGTHENED
    positionWeight: 3.0,            // MAXIMUM positional play (2.5→3.0)
    initiativeBonus: 95,            // ULTRA-HIGH initiative value (85→95)
    pieceActivityBonus: 90,         // Piece activity PARAMOUNT (80→90)
    controlBonus: 75,               // Space and control critical (65→75)
    mobilityWeight: 2.6,            // Piece mobility ESSENTIAL (2.2→2.6)
    coordinationWeight: 2.5,        // Piece coordination VITAL (2.1→2.5)
    
    // AUTHENTIC ALPHAZERO - Maximum creativity and freedom (REFINED)
    ...
    longTermFocus: 0.99,            // 99% focus on long-term positional play (0.98→0.99)
    eleganceThreshold: 0.55,        // Strong favor for elegant moves (0.50→0.55)
    openingScoreDiffThreshold: 80,  // More flexible opening creativity
    pieceSafetyWeight: 3.5,         // NEW: Critical weight for piece safety (prevent hanging pieces)
    ...
    // AUTHENTIC ALPHAZERO PERSONALITY - Fearless and Creative
    contempt: 60,                   // Balanced contempt for optimal play
    riskTolerance: 0.90,            // HIGH risk tolerance - fight for wins
    aggressivePlanning: 0.95,       // MAXIMUM aggressive long-term planning
    ...
    // AUTHENTIC ALPHAZERO: Advanced positional parameters
    pawnStructureWeight: 1.8,       // Higher pawn structure importance
    kingSafetyWeight: 1.4,          // REDUCED - less paranoid about king safety
    outpostBonus: 55,               // HIGHER bonus for piece outposts
    spaceControlWeight: 1.7,        // HIGHER space control importance
    prophylacticBonus: 45,          // HIGHER bonus for prophylactic moves
    nonObviousBonus: 50,            // MUCH HIGHER - favor elegant, non-obvious moves
    openingCreativity: 0.25,        // INCREASED opening creativity (was 0.08)
...
/**
 * NEW v10.0.0: CRITICAL - Detect if a move leaves pieces hanging
 * This prevents the Queen hanging blunder and similar tactical oversights
...
function detectHangingPieces(proposedMove, alternatives) {
    try {
        ...
        // CRITICAL: If this move causes eval to drop by >200cp, likely hanging piece
        if (scoreDrop > 200) {
            debugLog("[SAFETY]", `🚨 WARNING: Move ${proposedMove} drops eval by ${scoreDrop}cp!`);
            
            // Check if this looks like a blunder (sudden large loss)
            const isBlunder = scoreDrop > 300;
            const isSevereLoss = scoreDrop > 400;
        ...
        // Additional check: If we're evaluating alternatives, make sure we're not missing
        // an obviously better move (>100cp difference)
        if (scoreDrop > 100 && topScore > -200) {
            debugLog("[SAFETY]", `⚠️ Move ${proposedMove} significantly worse (${scoreDrop}cp) - reconsider`);
            return { safe: false, hangingPiece: "positional", evaluation: scoreDrop };
        }
...
/**
 * NEW v10.0.0: Validate move safety comprehensively
 * Ensures move doesn't leave valuable pieces undefended
 */
function validateMoveSafety(move, alternatives, currentEval) {
    try {
        ...
        if (!safetyCheck.safe) {
            ...
            // In non-critical positions, reject unsafe moves
            if (currentEval > -300) {
                return false;
            }
            
            // Even in bad positions, don't make it worse by >400cp
            if (safetyCheck.evaluation > 400) {
                return false;
            }
        }
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // AUTHENTIC ALPHAZERO: Creative opening play (first 10 moves)
    if (moveCount <= 10) {
        debugLog("[ENGINE]", `📖 OPENING PHASE (move ${moveCount}) - CREATIVE ALPHAZERO PLAY`);
        ...
        // Apply moderate opening creativity (25% - INCREASED)
        if (Math.random() > CONFIG.openingCreativity) {
            debugLog("[ENGINE]", "📖 OPENING: Playing best move");
            return bestMove;
        }
        
        // Allow creative alternatives within reasonable bounds
        if (scoreDiff < 35 && validateMoveForPosition(alternatives[1].move, currentFen, secondScore, topScore)) {
            debugLog("[ENGINE]", `📖 OPENING: Creative alternative (Δ${scoreDiff})`);
...
function initializeChessEngine() {
    ...
    chessEngine.postMessage("setoption name Contempt value 60");       // AUTHENTIC: Balanced contempt for fighting chess
    ...
    console.log("🤖 AUTHENTIC ALPHAZERO v10.0.0 - SOLID GENIUS EDITION");
    console.log("✅ ENHANCED: Tactical solidity - no more hanging pieces");
    console.log("✅ ENHANCED: Depth 40-60 - deeper calculation for perfect play");
    console.log("✅ ENHANCED: Strengthened positional evaluation - mobility, coordination, activity");
    ...
    console.log("✅ AUTHENTIC ALPHAZERO: Free-playing, intuitive, long-horizon positional genius");
    console.log("🛡️ NEW v10.0.0: Piece safety validation prevents blunders");
}
